...
class TestCore(TestCase):
    def test_ticked_should_assign_core_to_the_first_task(self):
        ...
        Core(tasks_pool=pool, mode=Mode.SYNC)
        core2 = Core(tasks_pool=pool, mode=Mode.ASYNC)
        ...
        self.assertEqual(task1, core2.processing)
    ...
    def test_ticked_should_stick_sync_core_with_task_until_its_finished(self):
        for to_process in [True, False]:
            ...
            core = Core(tasks_pool=pool, mode=Mode.SYNC)

            # then
            core.ticked()
            self.assertEqual([], pool)
            self.assertEqual(task, core.processing)

            core.ticked()
            self.assertEqual([], pool)
            self.assertEqual(None, core.processing)
    ...
    def test_ticked_should_stick_core_with_task_until_its_part_finished_and_return_task_to_the_pool(self):
        ...
        core = Core(tasks_pool=pool, mode=Mode.ASYNC)
